%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%                A Golog interpreter in iProlog
%
%
% (Adapted from the ECLiPSe version by Claude Sammut)
%
%
%                             March 2010
%
...
do((if P then E1 else E2), S, S1) :- do((?(P) : E1) # (?(-P) : E2), S, S1).
do(star(E), S, S1) :- S1 = S ; do(E : star(E), S, S1).
do(while(P, E), S, S1):- do(star(?(P) : E) : ?(-P), S, S1).
do(while(P, E), S, S1):- do(star(?(P) : E) : ?(-P), S, S1).
% do(pi(V, E), S, S1) :- sub(V, _, E, E1), do(E1, S, S1).
do(pi(V, E), S, S1) :- do(E, S, S1).		% Use Prolog variables and save the fussß
...
/* Don't need this at all! Just use Prolog variables.				*/
...
isAtom(A) :- functor(A, F, _), \+ member(F, [-, &, =>, <=>, v, some, all]).
