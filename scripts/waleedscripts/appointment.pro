...
ppevent(X) is [ "On", ppdateli(s_date(X)), "from", pptime(s_start(X)), " to ", pptime(s_end(X)), "about: ", s_desc(X), " -- " ]. 
...
go :- coord_connect, probot(c_frontend, [init]). 
