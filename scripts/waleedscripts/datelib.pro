...
ppdateli(date(Year, Month, Day)) is 
             [dowlong(calcday(date(Year, Month, Day))), " -- " , Day, monthnamelong(Month)]. 


pptime(time(Hours, 0, 0)) is [H, P] :-
	H is totwelve(Hours), 
	P is period(Hours). 

pptime(time(H, 15, 0)) is [quarter, past, NH, P]  :-
	NH is totwelve(H), 
	P is period(H). 

pptime(time(H, 30, 0)) is [half, past, NH, P]  :-
	NH is totwelve(H), 
	P is period(H). 

pptime(time(H, 5, 0)) is [five, past, NH, P]  :-
	NH is totwelve(H), 
	P is period(H). 

pptime(time(H, 10, 0)) is [ten, past, NH, P]  :-
	NH is totwelve(H), 
	P is period(H). 

pptime(time(H, 20, 0)) is [twenty, past, NH, P]  :-
	NH is totwelve(H), 
	P is period(H). 

pptime(time(H, 55, 0)) is [five, to, NH, P]  :-
	NH is totwelve(H+1), 
	P is period(H). 

pptime(time(H, 50, 0)) is [ten, to, NH, P]  :-
	NH is totwelve(H+1), 
	P is period(H). 

pptime(time(H, 45, 0)) is [quarter, to, NH, P]  :-
	NH is totwelve(H+1), 
	P is period(H). 

pptime(time(H, 40, 0)) is [twenty, to, NH, P]  :-
	NH is totwelve(H+1), 
	P is period(H). 

pptime(time(Hours, Minutes, Seconds)) is [H,  Minutes, P] :-
	H is totwelve(Hours),
                P is period(Hours). 
...
setscratchtime(X) :- assert(tmptime(X)). 
...
gstimepm is time(NewHours, Mins, 0) :-
	tmptime(X), 
	time(Hours, Mins, 0) is X, 
	NewHours is Hours + 12.

gottime(Y) :-
	time(H,M,S) is getprop(global, tmptime).
	
makepm(time(H,M,S)) is time(PH, M, S) :-
	PH is H + 12. 

makeam(time(H,M,S)) is time(PH, M, S) :-
	PH is H. 
