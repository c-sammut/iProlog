...
static term 	_of, _from, _to, _by, _replace, _remove, _add, _dot_dot, _slot,
...
void make_slot(term name, char *slot, term value)
{
	term cl = new_h_fn(3);
	ARG(0, cl) = _value;
 	ARG(1, cl) = name;
...
static void new_slot(term name, term slot, term facet, term value)
{
	term cl = new_h_fn(3);
	ARG(0, cl) = facet;
	ARG(1, cl) = name;
...
static term remove_slot(term name, term slot)
{
	term *p;
...
static term find_clause(term name, term slot, term facet)
{
	term p, q;
...
static term lookup(term name, term slot, term facet)
{
...
term slot_value(term name, term slot)
{
...
static term find_daemon(term obj, term attr, term facet)
{
	term p, rval = NULL;
...
int fput_is(term slot, term val, term *var_frame)
{
	SAVE(var_frame);
...
static int fput(term goal, term *var_frame)
{
...
static int freplace(term goal, term *var_frame)
{
	term x = check_arg(1, goal, var_frame, FN, IN);
	SAVE(var_frame);
...
static int fremove(term goal, term *var_frame)
{
...
static term get_value(term obj)
{
	term rval, p;
...
static term fget(term goal, term *var_frame)
{
	SAVE(var_frame);
...
static term have_property(term expr, term *var_frame)
{
...
static void create_daemons(term obj, term attr, term val)
{
	if (TYPE(val) != FN)
...
static void create_values(term obj, term attr, term val)
{
	this.object = obj;
	this.slot = attr;
	this.new_value = val;
...
static void traverse(term obj, term plist,  void (*what_to_do)())
{
	if (plist == NULL)
		return;
	if (TYPE(plist) != FN)
...
static term new_frame(term name, term body, void (*what_to_do)())
{
...
static bool new_generic(term goal, term *var_frame)
{
...
static int new_instance(term goal, term *var_frame)
{
	term name = check_arg(1, goal, var_frame, ATOM, IN);
	term body = check_arg(2, goal, var_frame, ANY, IN);
	SAVE(var_frame);
...
static term make_frame(term goal, term *var_frame)
{
	term name = make_name();
...
static bool print_daemon(term goal, term *frame)
{
...
static term ask(term goal, term *frame)
{
	int i;
...
static term p_this(term goal, term *frame)
{
	term x = check_arg(1, goal, frame, ATOM, IN);
...
static term new(term goal, term *frame)
{
	term x = check_arg(1, goal, frame, ATOM, IN);
...
static term old(term goal, term *frame)
{
	term x = check_arg(1, goal, frame, ATOM, IN);
...
static bool must_be_help(term g, term parents)
{
	term p;
...
static bool must_be_a(term goal, term *frame)
{
...
//	new_pred(interval, "..");
	...
	fprintf(stderr, "Old Frame Library\n");
}
